...
import {
    ArrowLeft, Copy, Download, ChevronDown, FileText,
    Bold, Italic, Underline as UnderlineIcon, Strikethrough,
    Heading1, Heading2, Heading3,
    List, ListOrdered, Quote,
    Code, Minus, Undo2, Redo2, Link as LinkIcon
} from 'lucide-react';
...
const GenerateEdit = () => {
    ...
    return (
        <div className="app-layout generate-edit-page">
            <Sidebar />
            <div className="main-content-wrapper">
                <main className="generate-edit-content">
                    {/* Page Header */}
                    <div className="page-header">
                        <div className="page-header-left">
                            <button className="back-link" onClick={() => {
                                const lastIntegration = sessionStorage.getItem('last_integration');
                                if (lastIntegration === 'devrev') {
                                    navigate('/devrev-dashboard');
                                } else {
                                    navigate('/dashboard');
                                }
                            }}>
                            ...
                            <h1 className="page-title">Generate & Edit</h1>
                        </div>
                        <div className="page-header-actions">
                            <button onClick={handleCopy} className="dropdown-trigger" type="button" title="Copy to Clipboard">
                            ...
                            <div className="download-wrapper">
                                <button
                                    ...
                                    className={`dropdown-trigger ${showDownloadMenu ? 'open' : ''}`}
                                    type="button"
                                >
                                    <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                        <Download size={16} />
                                    ...
                                    <ChevronDown size={14} className={`chevron ${showDownloadMenu ? 'rotated' : ''}`} />
                                </button>
                                {showDownloadMenu && (
                                    <div className="dropdown-options">
                                        <button className="dropdown-option" onClick={handleDownloadMd} type="button">
                                            <span style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                                <FileText size={16} />
                                                Markdown (.md)
                                            </span>
                                        </button>
                                        <button className="dropdown-option" onClick={handleDownloadDoc} type="button">
                                            <span style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                                <FileText size={16} />
                                                Word Document (.doc)
                                            </span>
                                        </button>
                                        <button className="dropdown-option" onClick={handleDownloadPdf} type="button">
                                            <span style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                                <FileText size={16} />
                                                PDF (.pdf)
                                            </span>
